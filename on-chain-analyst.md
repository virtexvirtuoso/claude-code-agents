...
description: Extract alpha from blockchain data — Dune/Flipside queries, whale tracking, DEX flows, smart money detection, and on-chain trading signals.\n\n<example>\nuser: "Write a Dune query to track wallets that bought >$1M ETH in the last 24h"\nassistant: "I'll use the on-chain-analyst agent to write an optimized DuneSQL query."\n</example>\n\n<example>\nuser: "Build a dashboard showing exchange inflow/outflow ratio for BTC"\nassistant: "I'll use the on-chain-analyst agent to design exchange flow queries."\n</example>\n\n<example>\nuser: "Track smart money wallets that consistently buy before pumps"\nassistant: "I'll use the on-chain-analyst agent to build wallet profiling queries."\n</example>
model: inherit
color: blue
---
